...
export function parseRepoDisplay(config: string): RepoDisplay {
	try {
		...
		return {
			color:
				typeof cfg.display_color === "string" ? cfg.display_color : undefined,
		};
	} catch {
		return {};
	}
...
export function setRepoDisplayColor(
	config: string,
...
export const api = {
	...
	getRoadmap: (repoId: string) =>
		invoke<{ content: string; exists: boolean; path: string }>("get_roadmap", {
			repoId,
		}),
	regenerateRoadmap: (repoId: string) =>
		invoke<{ content: string; exists: boolean; path: string }>(
			"regenerate_roadmap",
			{ repoId },
		),
	...
	completeItem: (id: string, note?: string) =>
		invoke<Item>("complete_item", { id, note }),
	cancelItem: (id: string, reason?: string) =>
		invoke<Item>("cancel_item", { id, reason }),
	...
	agentListRuns: (repoId?: string) =>
		invoke<AgentRun[]>("agent_list_runs", { repoId }),

	validateRepo: (repoId: string) =>
		invoke<ValidationReport>("validate_repo", { repoId }),
	fixRepo: (repoId: string) => invoke<FixReport>("fix_repo", { repoId }),
	...
	depsCheck: (repoId: string) => invoke<DepAnalysis>("deps_check", { repoId }),
	...
	discoverMdFolders: (path: string) =>
		invoke<DiscoveredFolder[]>("discover_md_folders", { path }),
	writeRoadmapJson: (repoPath: string, configJson: string) =>
		invoke<void>("write_roadmap_json", { repoPath, configJson }),
};
